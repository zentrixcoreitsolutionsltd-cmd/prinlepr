...
export const CaseStudiesSection: React.FC<{ onOpenConsultation: () => void }> = ({
  ...
  const cases = [
    ...
    },
    {
      ...
      category: 'Biotechnology & FDA Regulatory PR',
  ...
  return (
    <section id="cases" className="py-16 lg:py-24 bg-slate-50 border-t border-b border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        
        {/* Section Heading */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            <span className="text-[#e5a828] font-bold text-xs sm:text-sm tracking-[0.2em] uppercase block mb-2">
              PROVEN RESULTS
            </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          {cases.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-lg p-6 sm:p-7 border border-slate-200/80 shadow-xs hover:shadow-md transition-all flex flex-col justify-between"
            >
              <div>
                <span className="text-[11px] font-bold text-[#d89e28] tracking-wider uppercase block mb-2">
                  {item.category}
                </span>
                <h3 className="font-extrabold text-[#0d2137] text-lg sm:text-xl leading-snug mb-3">
                  {item.headline}
                </h3>
                ...
                {/* Key Metrics */}
                <div className="grid grid-cols-2 gap-3 py-4 border-t border-b border-slate-100 bg-slate-50/70 rounded-none px-3 mb-6">
                  <div>
                    <span className="block text-2xl font-black text-[#0d2137] tracking-tight font-sans">
                      {item.statPrimary}
                    </span>
                  ...
                  <div>
                    <span className="block text-2xl font-black text-[#d89e28] tracking-tight font-sans">
                      {item.statSecondary}
                    </span>
                    <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">
              ...
              <button
                ...
                className="inline-flex items-center justify-between text-xs font-bold text-[#0d2137] hover:text-[#d89e28] transition-colors pt-2 cursor-pointer"
              >
      ...
      {selectedCase && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-xl w-full p-6 sm:p-8 shadow-2xl border border-slate-100 relative animate-fadeIn max-h-[90vh] overflow-y-auto">
            <button
            ...
            <span className="text-xs font-bold text-[#df9e2c] uppercase tracking-wider">
              {selectedCase.category}
            </span>
            <h3 className="text-2xl font-extrabold text-[#0a1c36] mt-1 mb-2">
              {selectedCase.client}
            </h3>
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 rounded-lg border border-slate-200/60 mb-6">
              <div>
                <span className="text-3xl font-black text-[#0a1c36]">{selectedCase.statPrimary}</span>
                <span className="text-xs text-slate-500 block uppercase font-medium mt-0.5">
                  {selectedCase.statLabel}
                </span>
              </div>
              <div>
                <span className="text-3xl font-black text-[#df9e2c]">{selectedCase.statSecondary}</span>
                <span className="text-xs text-slate-500 block uppercase font-medium mt-0.5">
            ...
            <div className="space-y-4 text-xs sm:text-sm text-slate-700 mb-8">
              <div>
                <strong className="block text-[#0a1c36] uppercase text-xs tracking-wider mb-1">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex-1 bg-[#df9e2c] hover:bg-[#cb8b20] text-white font-bold text-xs uppercase tracking-wider py-3 rounded-sm text-center"
              >
                Discuss A Similar Campaign
              </button>
              <button
                onClick={() => setSelectedCase(null)}
                className="px-4 py-3 border border-slate-200 text-slate-600 hover:text-slate-900 text-xs font-bold rounded-sm"
              >
                Close
              </button>
            </div>
