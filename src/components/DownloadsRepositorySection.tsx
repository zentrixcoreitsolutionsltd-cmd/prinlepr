...
import { Download, FileText, CheckCircle2, Shield, Lock, ArrowDownToLine, AlertCircle } from 'lucide-react';
...
export const DownloadsRepositorySection: React.FC = () => {
  ...
  return (
    <section id="downloads" className="py-16 lg:py-24 bg-white border-b border-slate-200">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {assets.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="bg-slate-50 border border-slate-200 p-6 sm:p-7 flex flex-col justify-between hover:border-[#d89e28] transition-colors shadow-2xs"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-black uppercase tracking-widest bg-[#0d2137] text-white px-2.5 py-1">
                        {item.type}
                      </span>
                  ...
                  <h3 className="text-lg font-bold text-[#0d2137] mb-2 leading-snug">
                    {item.title}
                  </h3>
                ...
                <div className="pt-4 border-t border-slate-200 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`text-xs font-bold uppercase tracking-wider px-4 py-2 flex items-center gap-2 transition-all cursor-pointer ${
                      isDone
        ...
        {/* Compliance Notice: Mandate from Section 4 */}
        <div className="mt-8 p-4 bg-slate-100 border border-slate-200 flex items-start gap-3 text-xs text-slate-600">
